...
TEST(SQLite_ConnectionPool, T03_ErasesSingleTargetEntry)
{
    ...
    std::lock_guard<std::mutex> Lock(SQLite_ConnectionPool::Mutex);
    EXPECT_EQ(SQLite_ConnectionPool::Connections.count(Path1), 0);
    EXPECT_EQ(SQLite_ConnectionPool::Connections.count(Path2), 1);
}
...
TEST(SQLite_ConnectionPool, T04_ClearsAllConnections)
{
    ...
    std::lock_guard<std::mutex> Lock(SQLite_ConnectionPool::Mutex);
    EXPECT_TRUE(SQLite_ConnectionPool::Connections.empty());
}
