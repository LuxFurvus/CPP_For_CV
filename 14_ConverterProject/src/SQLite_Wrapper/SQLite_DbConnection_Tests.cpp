...
// T12: throws when Get() is called on a connection with a null internal handle (corrupt state)
...
TEST(SQLite_DbConnection, T12_CorruptGetShouldCrash)
{
    ...
    public:
        CorruptDbConnection(const char* Path) : SQLite_DbConnection({Path, false, true}) {
