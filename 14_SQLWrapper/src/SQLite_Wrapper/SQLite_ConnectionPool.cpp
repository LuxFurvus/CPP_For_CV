...
bool SQLite_ConnectionPool::IsConnectionOpen(const std::string &DbPath)
{
    std::lock_guard<std::mutex> Lock(Mutex);
    return Connections.contains(DbPath);
}

std::vector<std::string> SQLite_ConnectionPool::GetOpenConnectionPaths()
{
    std::vector<std::string> Result;
    std::lock_guard<std::mutex> Lock(Mutex);
    for (const auto& [OneDbPath, _] : Connections)
    {
        Result.emplace_back(OneDbPath);
    }
    return Result;
}
